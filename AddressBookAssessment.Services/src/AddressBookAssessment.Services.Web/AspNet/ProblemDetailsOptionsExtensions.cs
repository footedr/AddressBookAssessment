...
{
	public static ProblemDetailsOptions MapExceptionToStatusCode<T>(this ProblemDetailsOptions options, int statusCode, string? title = null, string? type = null, Func<T, string>? detail = null, Func<T, (string, object)>? extension = null) where T : Exception
	{
		return options.MapException<T>((problemDetails, exception) =>
		{
			...
			problemDetails.Detail = detail == null ? exception.Message : detail(exception);

			if (extension is not null)
			{
				var value = extension(exception);
				problemDetails.Extensions.Add(value.Item1, value.Item2);
			}
		});
	}

	public static ProblemDetailsOptions MapException<T>(this ProblemDetailsOptions options, Action<ProblemDetails> action) where T : Exception => options.MapException<T>((problemDetails, _) => action(problemDetails));
	...
	public static ProblemDetailsOptions MapException<T>(this ProblemDetailsOptions options, Action<ProblemDetails, T> action) where T : Exception
	{
		...
		options.CustomizeProblemDetails = ctx =>
		{
			...
			if (exceptionFeature?.Error is T exception)
			{
